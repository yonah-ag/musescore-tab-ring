 ...
 *
 * Copyright © 2023, 2024 yonah_ag, scorster
 *
 ...
 *
 *  12 Nov 2023 | 1.0   | Initial release
 *  09 Dec 2023 | 1.1   | Fixed picking gap sometimes not set
 *  12 Dec 2023 | 1.1.1 | Enabled Ring Tokens on rests
 *  13 Dec 2023 | 1.1.2 | Fixed picking gap sometimes set when not required
 *  01 Jan 2024 | 1.2   | MuseScore 3.6.2 full ring compatibility mode
 */
...
// Parameters
   ...
   property var zMuco : false // MuseScore 3.6.2 compatibility mode
...
//=================================================================================================================================
   ...
   function ringNote(ix,note,noto) // current note index and object
   {
      ...
      var dollType = 0;
      var zTokn = false; // the note was stopped by a Ring Token
      var zNoss = false; // the note was stopped by the Next Note on the Same String
      var disFret = 0; // dissonant fret span

      if (zLog) tlog = "<tr><td>" + ix + "<td><td>" + noteTick + "<td>" + onTk + "<td>" + noteFval
      + "<td>" + noteStrg + "." + mFret[ix] + "<td>" + mTokn[ix];
      ...
      if (zFace) {
         ...
         if (noteFval >= maxRing) { // if current note length >= max
            ...
            while (noteRing == 0) {
               ...
               if (nextTick == tickEnd) {
                  ...
                  nextOnTk = mOnTk[ix];
                  if (zLog) tlog += "<tr><td>&nbsp;<td>" + ix + "<td>" + nextTick
                  + "<td><td><td>"+mStr[ix]+"."+mFret[ix]+"<td>"+mTokn[ix];
               }
               ...
               if (nextTick >= maxTick) { // if next tick at or beyond limit
               ...
               else if (mTokn[ix] > 0) { // if $ Ring Token
                  dollType = mTokn[ix];
                  switch (dollType) {
                  case 1: // $NONE
                     zTokn = true;
                  ...
                  case 4: // $FRET
                     ...
                     break;
                  default:
                     if ((bitStop[noteStrg] & mTokn[ix]) > 0) {
                        zTokn = true;
                        noteRing = nextTick - noteTick;
                        if (zLog) tlog += "<td>$"+dollType;
                        break;
         ...
         if (noteRing > 0) {
            ...
            if (noteLen < 200) noteLen = 200; // don't let notes get too small
            if (zMuco) noteLen = Math.floor(noteLen / 100); 
...
//=================================================================================================================================
   ...
   function mapScore() // Build score map variables
   {
      ...
      while (mez) {
         ...
         mez = mez.nextMeasure;
         tokn = dolMez; // default tokn for next measure
      }
      ...
      if (zLog) {
         ...
         for (ii=0; ii < nofN; ++ii) {
            ...
            if (tick >= tickFr && tick < tickTo) {
               ...
               tout += "<td>&nbsp;" + mMIDI[ii] + "<td>&nbsp; " + mPlay[ii] + "<td>" + mOnTi[ii] + ", " + mLen[ii];
               tout += "<td>" + mOnTk[ii] + ", " + mLnTk[ii] + "<td>&nbsp;" + mTokn[ii] + "</tr>";
            }
         }
...
//=================================================================================================================================
   ...
   function checkScore()
   {
      if (mscoreVersion == 30700 || mscoreVersion == 30602) {
         ...
         isTab = 0;
         tdet = "MuseScore " + mscoreVersion + " is not supported";
      }
      if (isTab == 0) {
         runInfo.text = tdet;
...
//=================================================================================================================================
   ...
   function ringMu362() // MuseScore 3.6.2 workaround for full let ring
   {
      mscxPath = curScore.path.substring(0, curScore.path.lastIndexOf("/")+1) + curScore.scoreName+"_x.mscx";
      writeScore(curScore, mscxPath, "mscx");
      var mscxFile = fid.read();
      mscxFile = mscxFile.replace(/<len>(.+?)</g, '<len>$100<');
      fid.write(mscxFile);
      mscxFile = '';
      readScore(mscxPath, true);
      cmd("undo"); // undo compatibilty mode duration compression
   }
   ...
   function runMain(reset)
   {
      ...
      if (isTab > 0) {
         ...
         if (mscoreVersion == 30602) {
            zMuco = chkRing362.checked;
            if(zMuco)
               ringLim = 60000; // MS3.6.2 compatibility mode
            else
               ringLim = 2000;  // MS3.6.2 native mode
         }
         else {
            zMuco = false;
            ringLim = 60000; // MS3.7.0 native mode
         }
         ...
         if (zMuco)
            ...
            cmd("play"); // start play - forces PRE refresh
            cmd("play"); // stop play
         }
...
// USER INTERFACE
   ...
   Rectangle { id: uiRow362
      x: 15; y: uiRow6.y+30
      width: 300; height: 22; color: "transparent"
      MouseArea {
         anchors.fill: parent
         onClicked: { chkRing362.checked = !chkRing362.checked }
      }
      enabled: true
   }
   RowLayout { id: uiRow362a
      x: 15; y: uiRow362.y
      CheckBox { id: chkRing362;
         Layout.preferredWidth: 15
         checked: false
         enabled: true
         text: ""
      ...
      Label { id: lblRing362
         Layout.preferredWidth: 120
         color: "#000000"
         text: "MuseScore 3.6.2 full ring compatibility mode"
      }
   }
   RowLayout { id: uiRow7
      x: 15; y: uiRow362.y+40
      Button { id: btnDelPE
         Layout.preferredWidth: 110
   ...
   TextArea { id: runInfo
      ...
      text: "TAB Ring: " + version + "\nMuseScore: " + Math.floor(mscoreVersion/10000) + "." + Math.floor((mscoreVersion % 10000)/100) + "." + (mscoreVersion % 100)
   }
   ...
   Settings {
      ...
      property alias muCo362 : chkRing362.checked
   }
}